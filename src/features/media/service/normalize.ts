...
class FfmpegWasmNormalizeAdapter implements VideoNormalizeAdapter {
  ...
  private async ensureLoaded(signal?: AbortSignal) {
    ...
    const [ffmpegMod, utilMod] = await loadFfmpegModules();
    throwIfAborted(signal);
...
async function loadFfmpegModules(): Promise<[FfmpegModule, FfmpegUtilModule]> {
  const importModule = (specifier: string) =>
    Function("m", "return import(m)")(specifier) as Promise<unknown>;

  try {
    const ffmpegMod = (await importModule("@ffmpeg/ffmpeg")) as FfmpegModule;
    const utilMod = (await importModule("@ffmpeg/util")) as FfmpegUtilModule;
    return [ffmpegMod, utilMod];
  } catch {
