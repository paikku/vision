...
- 폐쇄망 대응: ffmpeg는 npm 패키지(`@ffmpeg/ffmpeg`, `@ffmpeg/util`, `@ffmpeg/core`)를 사용하며 `postinstall`에서 `public/vendor/ffmpeg/{core,ffmpeg,util}` 디렉토리를 복사해 로컬 모듈/코어/워커를 로드. 필요 시 `NEXT_PUBLIC_FFMPEG_MODULE_URL`, `NEXT_PUBLIC_FFMPEG_UTIL_URL`, `NEXT_PUBLIC_FFMPEG_CORE_URL`, `NEXT_PUBLIC_FFMPEG_WASM_URL`, `NEXT_PUBLIC_FFMPEG_WORKER_URL`로 오버라이드 가능
