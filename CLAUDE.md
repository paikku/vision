...
- 폐쇄망 대응: ffmpeg는 npm 패키지(`@ffmpeg/ffmpeg`, `@ffmpeg/util`, `@ffmpeg/core`)를 사용하며 `postinstall`에서 `public/vendor/ffmpeg/{ffmpeg-core.js,ffmpeg-core.wasm,worker.js}`를 복사해 로컬 경로로 로드. 필요 시 `NEXT_PUBLIC_FFMPEG_CORE_URL`, `NEXT_PUBLIC_FFMPEG_WASM_URL`, `NEXT_PUBLIC_FFMPEG_WORKER_URL`로 오버라이드 가능
