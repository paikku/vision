...
- **필터 row 구성** (단일 row): `[검색(w-44, placeholder=파일명)]` · `source [모두 업로드 프레임]` · `[resource ▾]` · `[tag ▾]` · 활성 필터가 1개라도 있으면 `[× 필터 초기화]` 가 우측에 노출. resource/tag 칩은 `FilterPopoverChip` 컴포넌트 — 0 선택이면 `{label} 전체`, 1개면 `{label}: {name}`, 2+ 면 `{label}: N개` 로 텍스트가 바뀐다. 클릭 시 popover 가 떠서 상단에 검색 input + `전체 선택` / `해제`, 그 아래 체크박스 리스트. `전체 선택` / `해제` 는 현재 필터링된(검색어 적용된) 부분집합에만 작용해서 큰 옵션 목록도 점진 선택 가능.
- **resource 필터 sync 모델**: ImagePool 의 resource 칩은 별도 state 가 아니라 `selectedResourceIds` 를 직접 토글한다 (`onResourceSelectionChange` prop). ResourcePool 의 행 토글과 ImagePool 의 popover 체크박스가 같은 set 을 갱신하므로 두 표면이 항상 일치. 사용자가 한 곳에서 클리어하면 다른 곳도 따라 클리어됨.
- **`× 필터 초기화` 동작**: `search` + `sourceFilter` + `tagFilter` + `selectedResourceIds` 를 한 번에 리셋. 직전에 있던 "image tags row" (별도 줄로 떠있던 image tag 칩 목록) 는 제거됨 — tag 필터는 popover 안에서만 다룸.
...
- **Bulk tag**: 푸터의 "태그 일괄…" 버튼으로 토글. 컴포넌트는 `src/components/BulkTagBar.tsx` (별도 파일, ImagePool 안이 아니라 `MediaLibraryPage` 가 sticky 푸터 위에 슬라이드 인 시킨다). 모드는 **세그먼트 라디오**(`추가 / 제거 / 교체`) — 한 번에 한 모드만 선택, 적용 버튼은 단일 `[{모드} 적용 (N장)]`. 입력 옆에 라이브 미리보기 텍스트(예: `선택된 47장에 \`cat\`, \`outdoor\` 를 추가합니다.`) 가 항상 표시돼 사용자가 클릭 직전 결과를 알 수 있다. `교체` 선택 시 라디오/적용 버튼이 danger 색으로 전환되고, 적용 시 native `confirm` 으로 한 번 더 확인 (기존 태그를 모두 지우는 destructive 동작이라). 성공 시 `onMutated()` 로 부모가 재로드.
- **Sticky CTA 푸터** (`MediaLibraryPage`): selection 액션은 ImagePool 안이 아니라 페이지 최하단 sticky 영역으로 모았다 — 사용자가 페이지 진입 즉시 "이미지 골라 → 라벨셋 만들기" 플로우를 인지할 수 있게. 항상 보이고, 선택이 0 이면 `선택된 이미지 없음 · 이미지를 골라 라벨셋을 만들 수 있어요` 안내가 뜨면서 `선택 해제` / `태그 일괄…` / `라벨셋 →` 가 disabled 되고, `현재 페이지 전체 선택` / `현재 결과 전체 선택` 만 활성 (entry point 라). 선택이 1+ 가 되면 accent 카운트 + 모든 액션 활성. 우측 끝에 강조 톤 `라벨셋 →` 가 단계감의 anchor.
- **ImagePool ↔ 푸터 동기화**: ImagePool 의 검색·필터·뷰모드 상태는 그대로 ImagePool 안에 있고, `onContextChange?: (ctx: ImagePoolContext)` 로 `{filteredIds, visibleIds}` 만 부모로 push. 부모는 이걸 `selectMany(visibleIds)` / `selectMany(filteredIds)` 액션의 source 로 쓰고, 카운트 표시도 같은 데이터에서 읽는다. id 배열은 `useMemo` 로 안정화돼 effect 가 무한 루프에 빠지지 않는다.
- **Start Labeling (`라벨셋 →`)**: 선택 1장 이상에서 활성화. `StartLabelingModal` 로 새 LabelSet 생성 또는 기존에 추가 (existing 의 경우 imageIds union → PATCH).
