...
- ffmpeg 모듈은 npm 패키지(`@ffmpeg/ffmpeg`, `@ffmpeg/util`, `@ffmpeg/core`) 기반으로 로드. `postinstall` 스크립트가 코어 파일을 `public/vendor/ffmpeg/*`로 복사하며 필요 시 `NEXT_PUBLIC_FFMPEG_CORE_URL`, `NEXT_PUBLIC_FFMPEG_WASM_URL`로 경로 오버라이드 가능
